...
**决策 9：将魔法词工坊重构为"Prompt 调味台"**
- 理由：原定位过于宽泛，缺乏清晰的使用场景和价值主张
- 新定位：专为严肃知识探索任务提供概念级补丁
- 任务边界：限定 4 类场景（分析/解释、架构设计、学习规划、产品拆解）
- 效果：产品角色从"生成器"变为"调味台" - 用户带着现成 Prompt 来，获得概念性增强

**决策 10：采用 lens 双维度架构**
- 理由：区分"深度"（vertical）和"广度"（horizontal）两种思维方式
- 实现：
  - vertical_lenses：往专业理论核心拉近，增强本质感
  - horizontal_lenses：从其他领域投射观察角度，增加立体感
- 数据结构：两个独立数组，各 5 个透镜
- UI 呈现：左右列布局，视觉上强化双维度概念

**决策 11：取消 patch_sentence 字段**
- 理由：用户更倾向于自由组合概念，而非使用预制的完整句子
- 实现：只保留 example_snippet，用户自行复制和改写
- 权衡：降低了"开箱即用"的便利性，但提升了灵活性

**决策 12：固定每维度 5 个透镜**
- 理由：
  - 2-4 个数量不稳定，影响 UI 布局和用户预期
  - 5 个是"足够多样化"与"不过载"的平衡点
- 实现：Prompt 模板明确要求"必须恰好 5 个"
- 风险：AI 可能不遵守，需后续监控和防御性解析
...
**坑 5：代码不一致导致的间歇性 500 错误**
- 现象：API 有时返回 500 错误，错误不稳定
- 根因：git merge 后不同文件使用了不一致的数据结构
  - `routes.ts` 期望新格式 `vertical_lenses/horizontal_lenses`
  - `prompts.ts` 和 `schema.ts` 仍返回旧格式 `magicWords`
- 解决：检查所有相关文件，确保数据结构定义、AI Prompt、解析逻辑三者一致
- 教训：重构数据结构时，必须同步更新整个调用链（schema → prompts → routes → frontend）

**坑 6：访问可能为 undefined 的字段导致前端崩溃**
- 现象：`Cannot read properties of undefined (reading 'length')`
- 根因：直接访问 `results.vertical_lenses.length` 但 API 返回可能缺少该字段
- 解决：使用可选链 `results.vertical_lenses?.length ?? 0`
- 教训：所有来自外部 API 的数据都应视为"可能不存在"，防御性编程
